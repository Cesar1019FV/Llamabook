...
export function PinnedSection() {
  ...
  const { chats, openChat } = useLlamabookDashboard()
        ...
        {pinned.map((item) => (
          ...
          >
            <div className="card-top flex items-center justify-between mb-[5px]">
              <span className="card-model text-[10px] text-llama-fg-5 font-normal px-1 py-px rounded bg-white/[0.08]">{item.model}</span>
              <span className="card-time text-[10px] text-llama-fg-5 font-normal">{formatRelativeTime(item.updated_at)}</span>
            </div>
